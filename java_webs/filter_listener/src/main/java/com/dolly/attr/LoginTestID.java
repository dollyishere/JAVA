...
import com.dolly.listen.LoginImpl;
...
public class LoginTestID extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// request.getSession은 기존 session 없을 시 객체 생성, 있을 시 기존 session 반환
 ...
 * 
 * setTimeout('호출함수', 대기시간)
 * 예) setTimeout('history.go(0);' 2000)
 * 		2초 후에 history.go(0) 함수 호출
 * 
 * history.go(0) => 웹 브라우저의 이전 / 다음 페이지로 이동
 * 				=> go(0) 현재 페이지로 이동
 * 				=> go(1) 다음 페이지로 이동
 * 				=> go(2) 다음다음 페이지로 이동
 * 				=> go(-1) 이전 페이지로 이동
